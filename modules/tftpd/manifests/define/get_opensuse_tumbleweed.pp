define tftpd::define::get_opensuse_tumbleweed($arch = [ "i386", "x86_64" ]) {
	...
	exec {
		...
		command     => "$download http://download.opensuse.org/tumbleweed/repo/oss/boot/$archi/loader/linux",
		...
		command     => "$download http://download.opensuse.org/tumbleweed/repo/oss/boot/$archi/loader/initrd",
